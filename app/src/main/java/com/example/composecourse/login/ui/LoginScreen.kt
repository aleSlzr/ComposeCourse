package com.example.composecourse.login.ui
...
fun Body(modifier: Modifier, loginViewModel: LoginViewModel) {
    ...
    Column(modifier = modifier) {
        ...
        Email(
            email = email,
            onTextChanged = { loginViewModel.onLoginChanged(email = it, password = password) }
        )
        Spacer(modifier = Modifier.size(4.dp))
        Password(
            password = password,
            onPasswordChange = { loginViewModel.onLoginChanged(email = email, password = it) }
        )
